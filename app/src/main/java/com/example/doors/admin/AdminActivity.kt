    ...
    private fun validateScannedCode(content: String) {
        try {
            repository.validateQr(content) { result ->
                when (result) {
                    is ValidationResult.Granted -> {
                        val movementVerb = if (result.movement == "Salida") "salió de" else "ingresó a"
                        showResultDialog(
                            iconRes = R.drawable.ic_result_success,
                            title = if (result.movement == "Salida") "Salida registrada" else "Entrada registrada",
                            message = "${result.visit.visitorName} $movementVerb el condominio. Visita al depto. ${result.visit.apartment}.",
                    ...
                    is ValidationResult.AlreadyUsed -> showResultDialog(
                        ...
                        message = "El QR de ${result.visit.visitorName} ya registró su entrada y salida (máx. ${result.visit.maxUses} usos) y no permite un nuevo ingreso.",
            ...
            // Última red de seguridad: pase lo que pase con el contenido escaneado,
            // nunca debe cerrar la pantalla de administración.
            showResultDialog(
                iconRes = R.drawable.ic_result_error,
                title = "⚠ QR no válido",
                message = "Este código no corresponde a ninguna visita registrada. Acceso denegado.",
